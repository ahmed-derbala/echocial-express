 ...
 */
 ...
 if (conf.app.cluster > 0) {
   ...
   if (cluster.isMaster) {
     ...
     server.listen(conf.app.backend.port, log({ message: `${conf.app.name} ${conf.app.version} ${conf.app.backend.url} NODE_ENV=${conf.NODE_ENV} fork ${cluster.worker.id} pid ${cluster.worker.process.pid}`, level: 'startup' }))
     server.on('error', onError);
     server.on('listening', onListening);
   }
 } else {
   //launching the server without cluster
   server.listen(conf.app.backend.port, log({ message: `${conf.app.name} ${conf.app.version} ${conf.app.backend.url} NODE_ENV=${conf.NODE_ENV}`, level: 'startup' }))
   server.on('error', onError);
   server.on('listening', onListening);
 }
