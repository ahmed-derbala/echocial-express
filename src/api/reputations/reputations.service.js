...
module.exports.createReputation = async ({ facebook, rating }) => {
    try{
    const fetchedReputation = await ReputationsModel.findOne({ facebook: { id: facebook.id } }).lean()
    if (fetchedReputation) return { error: true, message: `facebook.id=${facebook.id} already exists` }

    rating.ratersCount = 1
    return ReputationsModel.create({ facebook, rating })
        .then(createdReputation => {
...
module.exports.updateRating = async ({ reputationId, rating }) => {
    const fetchedReputation = await ReputationsModel.findOne({ _id: reputationId }).lean()
    ...
    return ReputationsModel.updateOne(
        { _id: reputationId },
        { rating })
